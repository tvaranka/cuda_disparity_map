...
//gcc -o stereo stereo.c lodepng.c -lm -lgomp -fopenmp -O3
...
void disp_map(double **img0, double **img1, int **disp_img0,
   ...
   #pragma omp parallel for private(best_d0, best_d1, zncc0, zncc1)
   for (int x = 0; x < height - win_size; x++){
      for (int y = d_max; y < width - win_size - d_max; y++){
